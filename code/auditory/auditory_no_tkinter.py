...
class ROIActivityMonitor:
    ...
    def load_sounds(self, folder_path):
        ...
        for roi_index in self.rois.keys():
            ...
            if os.path.isfile(sound_path):
                ...
                print("Loaded sound for ROI {}: {}".format(roi_index, sound_path))  # Debugging print
        return sounds
    ...
    def setup_camera(self):
        ...
        while True:
            ret, frame = vid.read()
            if not ret:
                print("Failed to capture frame")  # Error handling
                break
    ...
    def process_frame(self, frame, prev_frame):
        ...
        _, thresh = cv2.threshold(fg_mask, 25, 255, cv2.THRESH_BINARY)  # Thresholding
        ...
        for roi_index, ((x1, y1), (x2, y2)) in self.rois.items():
            ...
            print("Activity in ROI {}: {}".format(roi_index, activity_level))  # Debug output

            if activity_level > 5000:
                print("Triggering sound for ROI {}".format(roi_index))
    ...
    def play_sound(self, roi_index):
        ...
        if sound_path:
            try:
                ...
                print("Playing sound from: {}".format(sound_path))  # Debugging print
                sd.play(data, fs)
                sd.wait()
            except Exception as e:
                print("Failed to play sound: {}".format(e))
