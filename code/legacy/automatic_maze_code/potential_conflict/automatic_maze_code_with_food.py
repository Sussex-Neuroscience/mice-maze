...
try:
    time.sleep(1)
    pannel_sort('allzero')
    start_board.is_alive()
except:
    print("An error with the wall arduino was detected") 
...
try:
    ...
    test_tray()
    start_board_food.is_alive()
except:
    print("An error with the food arduino was detected") 
...
#Saving data ############################### Inializes here and closes by the end of the protocol
data_trial = [['time', 'RR_intensity', 'RL_intensity', 'LL_intensity', 'LR_intensity', 'trial_id']]
file_trial = open(path + AnimalID+'_'+current_time+'_'+training+'_datatrial.csv', 'w', newline='') 
writer = csv.writer(file_trial)
writer.writerow(data_trial[0])
Reward_time= [] 
...
trial_choice = "allzero"
# for i in list_labels:
#     exec('f'+i+' = open("'+i+'.csv", "w")')
# writerRR=csv.writer(fRR, delimiter='\t',lineterminator='\n',)
# writerRL=csv.writer(fRL, delimiter='\t',lineterminator='\n',)
# writerLL=csv.writer(fLL, delimiter='\t',lineterminator='\n',)
# writerLR=csv.writer(fLR, delimiter='\t',lineterminator='\n',)
        ...
        if grabbed == True:
            ...
            for i in range(0,4,1):
                ...
                if pixel > 15:
                    ...
                    #exec('writer'+list_labels[i]+'.writerow([time_base,time_base[-1]-time_base[-2]])')
                    if Reward[i]=='No':
                        Reward[i] ='Yes'
            ...
            if Rewarded_already == 'No':
                if Reward[index_trial] == "Yes":   
                    if Reward.count('Yes')==1:
                        ...
                        Reward_time.append(time_base[-1])
            ...
            ################### Save every 100 ms the last changes 
            if not isinstance(data_trial[0][0], float): #initialize 
                data_trial = [[time_base[-1], s_0[-1], s_1[-1], s_2[-1], s_3[-1], trial_choice]]
            if time_base[-1] + 200 > data_trial[0][0]:
                data_trial = [[time_base[-1], s_0[-1], s_1[-1], s_2[-1], s_3[-1], trial_choice]]
                writer.writerow(data_trial[0])
...
# for i in list_labels: #I don''t remember this code
#     exec('f'+i+'.close()')
...
print((s_0[-1],s_1[-1],s_2[-1],s_3[-1]))
file_trial.close()
file_reward = open(path + AnimalID+'_'+current_time+'_'+training+'_datareward.csv', 'w', newline='') 
writer = csv.writer(file_reward)
writer.writerow(Reward_time)
file_reward.close()