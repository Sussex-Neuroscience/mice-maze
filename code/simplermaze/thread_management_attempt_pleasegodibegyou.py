...
class ROIActivityMonitor:
    ...
    def process_frame(self, frame):
        ...
        for roi_index, ((x1, y1), (x2, y2)) in self.rois.items():
            ...
            self.activity_levels[roi_index] = activity_level
            if activity_level > max_activity:
                max_activity = max_activity
                most_active_roi = roi_index

        with self.lock:
            if most_active_roi and max_activity > 900000:
                if not self.active_sound_thread or not self.active_sound_thread.is_alive():
                    self.active_sound_thread = threading.Thread(target=self.play_sound_continuously, args=(most_active_roi,))
                    self.active_sound_thread.start()
            elif self.active_sound_thread and max_activity <= 900000:
                self.running = False
        cv2.imshow('Processed ROI Activity (BG Subtraction & Thresholding)', thresh_colored)

    def play_sound_continuously(self, roi_index):
        while self.running and self.activity_levels.get(roi_index, 0) > 900000:
            sd.play(self.sound_data[roi_index], blocking=False)
