...
if testing:
    ...
    trials = sf.create_trials(
        num_trials=100, experiment_phase=experiment_phase, nonRepeat=True
    )
    ...
    trials = sf.create_trials(num_trials=100, experiment_phase=experiment_phase)
...
sf.write_data(
    file_name=os.path.join(new_dir_path, f"session_data_{date_time}.csv"),
    mode="w",
    data=data.head(n=0),
)
...
for trial in trials.index:
    ...
    while trialOngoing:
        ...
        for item in rois:
            cv.rectangle(
                gray,
                (rois[item]["xstart"], rois[item]["ystart"]),
                (
                    ...
                    if mousePresent[item] and not rewarded:
                        ...
                        if not trials.wrongallowed[trial]:
                            for rewLoc in hasVisited:
                                if (
                                    hasVisited[rewLoc]
                                    and trials.rewlocation[trial] not in rewLoc
                                ):
                                    mistake = True
                        ...
                        if not mistake:
                            ...
                            data.loc[trial, "area_rewarded"] = trials.loc[
                                trial
                            ].rewlocation
                            data.loc[trial, "time_to_reward"] = (
                                time_frame - trial_start_time
                            )
